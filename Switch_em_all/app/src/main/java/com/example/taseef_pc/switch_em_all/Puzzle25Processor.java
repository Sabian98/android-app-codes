...
/**
 * Created by Taseef-PC on 9/12/2016.
 */
 ...
 */
public class Puzzle25Processor {
    ...
    private boolean mShowTileNumbers = false;

    public Puzzle25Processor(int d) {
    ...
    public synchronized Mat puzzleFrame(Mat inputPicture) {
        ...
        if(f==2){
            Imgproc.cvtColor(mRgba15, mRgba15, Imgproc.COLOR_BGR2BGRA );

        }
        else if(f==3){

            threshold(mRgba15,mRgba15,40,80,1);
        }
        drawGrid(cols, rows, mRgba15);
        return mRgba15;
    }
